...
###Set up SD and SE
Temp$sd <- sd(Temp$temperature, na.rm = TRUE)
se_temp <- sd(Temp$temperature)/sqrt(length(Temp$temperature))
Temp$se <- se_temp

###Get mean and max temp
Temp <- Temp %>%
  group_by(as.character(temp), species, pendant_ID, se, sd) %>%
...
TempFactor <- ggplot(final, aes(temp, number_germinated_6week),show.legend=FALSE) +
  geom_boxplot() +
  facet_wrap(~species)+
...
```{r}
###Plotting the lm
plot(lm(number_germinated_6week ~ mean_temp, data = final), which = 1)
```
```{r}
germination_poly <- lm(number_germinated_6week ~ mean_temp + I(mean_temp^2), data = final)
...
Temp <- ggplot(final, aes(mean_temp, number_germinated_6week),show.legend=FALSE) +
  geom_point() +
...
Tempspecies <- ggplot(final, aes(mean_temp, number_germinated_6week, color = species),show.legend=FALSE) +
  geom_point() +
  scale_color_brewer(palette = "Set1") + theme_classic() + 
  geom_smooth(method = lm, se = TRUE) + 
  labs(x = "Temperature", y = "Number of Germinated Seeds")
...
TempspeciesFacet <- ggplot(final, aes(mean_temp, number_germinated_6week, color = species),show.legend=FALSE) +
  geom_point() +
  facet_wrap(~temp)+
  scale_color_brewer(palette = "Set1") + theme_classic() + 
  geom_smooth(method = lm, se = TRUE) + 
...
TempspeciesFacet <- ggplot(final, aes(mean_temp, number_germinated_6week),show.legend=FALSE) +
  geom_point() +
  facet_wrap(~species)+
  scale_color_brewer(palette = "Set1") + theme_classic() + 
  geom_smooth(method = lm, se = TRUE) + 
...
TempspeciesFacetColor <- ggplot(final, aes(mean_temp, number_germinated_6week, color = temp),show.legend=FALSE) +
  geom_point() +
  facet_wrap(~species)+
  scale_color_brewer(palette = "Set1") + theme_classic() +
  geom_smooth(method = lm, se = TRUE) + 
...
m1 <- glm(number_germinated_6week ~ temp*species+mean_temp, family = "quasipoisson", data = final)
anova(m1, test = "Chisq")
...
m2 <- glm(mass ~ number_germinated_6week*temp*species+mean_temp, family = "quasipoisson", data = final)
anova(m2, test = "Chisq")
